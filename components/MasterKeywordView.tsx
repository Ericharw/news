...
export const MasterKeywordView: React.FC = () => {
  ...
  const handleAddKeyword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newKeyword.trim()) {
      alert("Harap isi Kata Kunci (Keyword).");
      return;
    }
    ...
    try {
      ...
      if (json.success) {
        ...
        alert(json.error || "Gagal menambah Master Keyword.");
      }
    } catch (err) {
  ...
  const confirmDeleteKeyword = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        alert(json.error || "Gagal menghapus Master Keyword.");
      }
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Master Keyword</h2>
